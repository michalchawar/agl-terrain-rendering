...
void AGLWindow::Init(int width, int height, const char *name, int fullscr, int vers) {
   ...
   // Bufor głębokości czyli z-bufor:
   // - uaktywniamy test:
   glEnable(GL_DEPTH_TEST);
   // - określamy funkcje LESS/LEQUAL/GEQUAL/..
   glDepthFunc(GL_LEQUAL);

   // Trywialne eliminowanie ścian tylnych
   // - uaktywniamy test:
   // glEnable(GL_CULL_FACE);
   // - określamy orientacje dla FRONT:
   glFrontFace(GL_CW); // domyślnie CCW
   // - określamy które odrzucać: BACK/FRONT
   glCullFace(GL_BACK); // domyślne
}
